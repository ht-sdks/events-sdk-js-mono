...
let simpleEvent: Record<string, any>
let manyPropertiesEvent: Record<string, any>
...
describe('allow_properties', () => {
  ...
  test('allow_properties should work on the top-level object (ill-advised as it is)', () => {
    ...
    expect(Object.keys(simpleEvent).length).toBeGreaterThan(1)
  ...
  test('allow_properties should drop nested fields and only in those fields', () => {
    ...
    transform(simpleEvent, [transformer])
    expect(Object.keys(simpleEvent)).toHaveLength(originalPropCount)
    expect(simpleEvent.nest1.nest2.nest3.nest4).toStrictEqual({
      nest5: 'hai :3',
    })
  })
  ...
  test('drop_properties should work quickly even on huge objects', () => {
    ...
    transform(manyPropertiesEvent, [transformer])
    expect(Object.keys(manyPropertiesEvent)).toHaveLength(originalPropCount)
    expect(manyPropertiesEvent.nest1.nest2.nest3.nest4).toStrictEqual({
      nest5: 'hai :3',
    })
  }, 500)
})
...
describe('map_properties', () => {
  ...
  test('map_properties should mutate the input object', () => {
    transform(simpleEvent, [transformer])
    expect(simpleEvent.mapMe).toBe(true)
  })
